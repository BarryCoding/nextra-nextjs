...
> Next.js has the preview mode for statically generated pages.

> **Note**: This feature is superseded by [Draft Mode](/next-page/building-your-application/configuring/draft-mode).
...
> Take a look at the [API Routes documentation](/next-page/building-your-application/routing/api-routes) first if you’re not familiar with Next.js API Routes.
...
> **Good to know**: during rendering `next/router` exposes an `isPreview` flag, see the [router object docs](/next-page/api-reference/functions/use-router#router-object) for more info.
...
Then, send a request to `/api/clear-preview-mode-cookies` to invoke the API Route. If calling this route using [`next/link`](/next-page/api-reference/components/link), you must pass `prefetch={false}` to prevent calling `clearPreviewData` during link prefetching.
...
> **Good to know**: You shouldn't set the `Cache-Control` header when using Preview Mode because it cannot be bypassed. Instead, we recommend using [ISR](/next-page/building-your-application/data-fetching/incremental-static-regeneration).
