...
pnpx create-next-app@latest --example with-jest with-jest-app
```
...
including automatically creating a `jest.config.ts` file.
...
- Auto mocking stylesheets (`.css`, `.module.css`, and their scss variants), image imports and [`next/font`](/next-page/building-your-application/optimizing/fonts)
...
> load them manually in a separate setup script or in your `jest.config.ts` file.  
> For more information, please see [Test Environment Variables](/next-page/building-your-application/configuring/environment-variables).

## Setting up Jest (with Babel)

If you opt out of the [Next.js Compiler](/docs/architecture/nextjs-compiler) and use Babel instead,  
you will need to manually configure Jest and install  
`babel-jest` and `identity-obj-proxy` in addition to the packages above.
...
We also recommend reviewing [`next/jest` configuration](https://github.com/vercel/next.js/blob/e02fe314dcd0ae614c65b505c6daafbdeebb920e/packages/next/src/build/jest/jest.ts) to see how Next.js configures Jest.
...
If your project is using [Module Path Aliases](/next-page/building-your-application/configuring/absolute-imports-and-module-aliases),  
you will need to configure Jest to resolve the imports by matching the paths option in the `tsconfig.json` file with the `moduleNameMapper` option in the `jest.config.js` file. For example:
...
> **Good to know:**  
> [`extend-expect` was removed in `v6.0`](https://github.com/testing-library/jest-dom/releases/tag/v6.0.0), so if you are using `@testing-library/jest-dom` before version 6,  
...
For more Jest CLI options, please refer to the [Jest Docs](https://jestjs.io/docs/cli#reference).
