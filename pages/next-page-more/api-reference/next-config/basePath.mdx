...
For example, to use `/docs` instead of `''` (an empty string, the default),  
...
> **Good to know**:  
> This value must be set at build time and cannot be changed without re-building  
> as the value is inlined in the client-side bundles.
