...
Most of the time, you will not need this - but it's available for complete customization.

> **Good to know**:
>
> - Before deciding to use a custom server, please keep in mind that it should only be used  
>   when the integrated router of Next.js can't meet your app requirements.  
>   A custom server will remove important performance optimizations, like **serverless functions** and **[Automatic Static Optimization](/docs/pages/building-your-application/rendering/automatic-static-optimization).**
...
> Make sure the syntax and sources this file requires are compatible with the node version.
...
> client-side routing may still access those paths.  
> When using this option, you should guard against navigation to routes you do not want programmatically.

> You may also wish to configure the client-side router to disallow client-side redirects to filename routes;  
> for that refer to [`router.beforePopState`](/docs/pages/api-reference/functions/use-router#routerbeforepopstate).
