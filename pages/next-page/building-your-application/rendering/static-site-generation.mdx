...
To learn more about how `getStaticProps` works, check out the [Data Fetching documentation](/next-page/building-your-application/data-fetching/get-static-props).

### page paths depend on data

Next.js allows you to create pages with **dynamic routes**.  
For example, you can create a file called `pages/posts/[id].js` to show a single blog post based on `id`.  
...
Later, you might add the second post with `id: 2`.  
Then you'd want to pre-render `posts/2` as well.
...
To learn more about how `getStaticPaths` works, check out the [Data Fetching documentation](/next-page/building-your-application/data-fetching/get-static-paths).
...
- Use Static Generation with **Client-side data fetching:**  
